...
class CreateQuestionsForm(forms.ModelForm):
    ...
    Question_Text = forms.CharField(max_length=500, widget=forms.Textarea(attrs={'class': 'form-control', 'rows':'5', 'id':'cDesc','name':'tDesc', 'placeholder':'Descrição do assunto do questionário'}))
...
class CreateOptionForm(forms.ModelForm):
    ...
    Option_Text = forms.CharField(max_length=500, widget=forms.Textarea(attrs={'class': 'form-control', 'rows':'5', 'id':'cAssQuiz','name':'tAssQuiz', 'placeholder':'Descrição  da alternativa'}))
